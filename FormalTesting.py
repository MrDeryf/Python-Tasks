def count_keys(state, character):
    count = 0
    for key, value in state.items():
        if "key" in key and value == character:
            count += 1
    return count


def change_character(new_character):
    def func(state):
        if new_character != state["player"]:
...
def go_through(door_color):
    def func(state):
        if state[f"{door_color}_key"] == state["player"] or state["player"] == "carl":
            if door_color in ["red", "blue"]:
                ...
                elif state["player"] == "carl":
                    return dict(state, carl_room=f"{door_color}_room")
                else:
                    return state
            else:
...
def pick(key_color):
    def func(state):
        if count_keys(state, state["player"]) < 2 and state["player"] != "carl":
